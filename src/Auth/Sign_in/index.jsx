    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-opacity-30">
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-md w-full max-w-md space-y-4 relative">
        <div className="flex justify-center mb-2 text-lg font-semibold">
          <button type="button" onClick={() => setActiveTab('login')} className={`px-4 ${activeTab === 'login' ? 'text-green-600 font-bold' : 'text-gray-400'}`}>Login
          </button>
            <span className="mx-2 text-gray-300">|</span>
          <button type="button" onClick={() => setActiveTab('register')} className={`px-4 ${activeTab === 'register' ? 'text-green-600 font-bold' : 'text-gray-400'}`}>Register
          </button>
        </div>
