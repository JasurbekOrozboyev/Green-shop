...
import { Slider, Button, IconButton, Menu, MenuItem } from '@mui/material';
...
const Category = () => {
    ...
    const getFlowers = async () => {
        try {
            ...
            console.log("Category.jsx: Gullarni olish uchun API URL:", apiUrl); 
            const res = await axios.get(apiUrl);
            let flowersData = res?.data?.data || [];
            console.log("Category.jsx: Gullar API javobi:", flowersData); 
    ...
    useEffect(() => {
        if (category || (data.length > 0 && !searchParams.get("category"))) {
            getFlowers();
        }
    }, [category, type, sort, minPrice, maxPrice, size, data]); 
            ...
            <div className="flex flex-col lg:flex-row justify-between gap-6 lg:gap-[50px]">
                ...
                <div className="flex flex-col border border-gray-200 rounded p-3 w-full">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-[50px] justify-items-center">
                        {flowers.length > 0 ? (
                            flowers.map((flower) => {
                                const cleanId = flower._id ? String(flower._id).replace(/[^a-zA-Z0-9]/g, '') : '';
                                console.log(`Category.jsx: Mahsulot "${flower.title}" uchun tozalangan ID:`, cleanId); 
                                if (!cleanId) {
                                    console.warn(`Category.jsx: Mahsulot "${flower.title}" uchun ID topilmadi yoki tozalash muvaffaqiyatsiz tugadi, o'tkazib yuborilmoqda.`);
                                    return null; 
                                }
                                return (
                                <div className="group w-full max-w-[280px]" key={flower._id}>
                                    <Link to={`/product/${cleanId}`}> 
                                        <div className="border border-gray-300 p-3 relative h-[300px] flex items-center justify-center overflow-hidden">
                                            ...
                                            <button
                                                className="w-7 h-7 absolute bottom-6 right-5 transition-all transform active:scale-75 bg-white rounded-full flex items-center justify-center shadow-md hover:bg-gray-100"
                                                onClick={(e) => {
                                                    e.preventDefault()
                                                    addToCart(flower);
                                                }}
